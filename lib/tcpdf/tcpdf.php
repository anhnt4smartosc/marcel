	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		if ($resth > 0) {
			...
			while ($tmpresth > 0) {
				...
				if ($this->num_columns > 1) {
					$tmpresth -= ($this->h - $this->y - $this->bMargin) < 0 ? -($this->h - $this->y - $this->bMargin) : ($this->h - $this->y - $this->bMargin);
				} else {
					$tmpresth -= ($this->h - $this->tMargin - $this->bMargin) < 0 ? -($this->h - $this->tMargin - $this->bMargin) : ($this->h - $this->tMargin - $this->bMargin);
				}
				--$skip;
			}
			$currentY = $this->y;
...
//			ob_end_clean();
//			Mage::app()->getResponse()->setHeader('Content-Type', 'application/pdf', true);
//		}
