...
class Magestore_Inventorydropship_Adminhtml_Indr_InventorydropshipController extends Mage_Adminhtml_Controller_Action {
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('inventoryplus/stock_out/dropship');
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('inventoryplus/stock_out/dropship');
    }
    ...
    public function uploadattachmentAction()
    {
        if ($this->getRequest()->getPost()) {
            ...
            if (!file_exists($targetDir)) {
                mkdir(Mage::getBaseDir("media") . DS . "inventorydropship", 0777, true);
            }
...
//                if ($check !== false) {
//                    echo $this->__("File is an image - " . $check["mime"] . ".");
//                    $uploadOk = 1;
//                } else {
//                    echo $this->__("File is not an image.");
//                    $uploadOk = 0;
//                }
//            }
...
//                && $imageFileType != "gif"
//            ) {
//                echo $this->__("Sorry, only JPG, JPEG, PNG & GIF files are allowed.");
//                $uploadOk = 0;
//            }
            ...
            if ($uploadOk == 0) {
                ...
                $newWebPath = $oldWebPath . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . "inventorydropship/" . basename($_FILES["fileToUpload"]["name"]) . ";";
                $dropship->setData('attachment_web_path', $newWebPath);
                try {
                    $dropship->save();
                    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $targetFile)) {
                        echo $this->__("The file " . basename($_FILES["fileToUpload"]["name"]) . " has been uploaded.");
                    } else {
